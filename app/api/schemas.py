...
class WordFrequencyQuery(BaseModel):
    ...
    depth: int = Field(
        0, ge=0, description="Traversal depth (0 = only the start article)"
    )

    model_config = {
        "json_schema_extra": {"examples": [{"article": "Earth", "depth": 2}]}
    }
...
class KeywordsRequest(BaseModel):
    ...
    ignore_list: List[str] = Field(
        default_factory=list, description="Words to exclude from the results"
    )
    percentile: int = Field(
        ...,
        ge=0,
        le=100,
        description="Keep words with count >= this percentile of counts",
    )
